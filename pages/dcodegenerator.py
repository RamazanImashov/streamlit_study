...
def sort_code(code: str):
    """
    Чтобы D2 < D10 (а не наоборот как при обычной строковой сортировке).
    """
    if not code:
        return ("", 10**9)
...
@st.cache_data(ttl=60)  # кэш на 60 сек, чтобы не дергать API каждый rerun
def load_clients():
    return get_clients()
...
def html_to_png_bytes(html: str) -> bytes:
    with sync_playwright() as p:
        ...
        # если в шаблоне есть внешние ресурсы (шрифты/картинки по URL), лучше:
